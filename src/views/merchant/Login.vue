        ...
        <v-col cols="12" sm="8" md="4">
          <v-card class="elevation-12">
            <v-toolbar color="primary" dark flat>
              ...
              <v-btn color="success" small @click="onSignupClicked">Signup</v-btn>
            </v-toolbar>
            <v-card-text>
            ...
            <v-card-actions>
              <v-btn small text color="primary" @click="onForgotPasswordClicked">Forget Password</v-btn>
              <v-spacer></v-spacer>
...
export default {
  ...
  methods: {
    ...
    onForgotPasswordClicked() {
      router.push("/merchant/forgot-password");
    },
    async onLoginClicked() {
      if (this.isLoginFormValid)
        try {
          ...
          let response = await this.$http.post("/auth/login/merchant", {
          ...
          store.dispatch("setMerchantAuthenticated", true);
          store.dispatch("setAuthenticatedMerchant", response.data.user);
          router.push("/merchant/dashboard/restaurant");
        } catch (err) {
          this.setLoading("Login...", false);
